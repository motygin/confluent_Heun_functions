...
function [C0,Cs,err,numb,wrnmsg] = HeunCjoin0infA(q,alpha,gamma,delta,epsilon,theta,varargin)
  ...
  if consts_known
    ...
    infpt = R0 * exp(1j*theta);
    joinpt = 1.25 * exp(1j*theta);

    if abs(joinpt)>abs(infpt)

      wrnmsg = 'HeunCjoin0infA: parameter epsilon is too large for the algorithm; ';
      C0 = NaN; Cs = NaN; err = NaN; numb = 0;
