...
function [val1,dval1,err1,val2,dval2,err2,numb,wrnmsg,val1woexp,dval1woexp,err1woexp,val2woexp,dval2woexp,err2woexp] = HeunCfaraway(q,alpha,gamma,delta,epsilon,z)
  ...
  if (imag(z)==0)&&((real(z)>=1)||(real(z)<=0))
    ...
    wrnmsg = 'HeunCfaraway: z belongs to the possible branch-cuts (-\infty,0], [1,\infty); ';
    ...
    elseif pi-abs(dir) < 0.01
    
      dir = pi + 0.01 * hs;
      
    end
    
    [C0A,CsA,errJA,numbJA,wrnmsgJA] = HeunCjoin0infA(q,alpha,gamma,delta,epsilon,dir);
    [C0B,CsB,errJB,numbJB,wrnmsgJB] = HeunCjoin0infB(q,alpha,gamma,delta,epsilon,dir);
  
    [R,N] = findR();
    infpt = max(1,R/(abs(eps)+abs(epsilon))) * z/abs(z);
